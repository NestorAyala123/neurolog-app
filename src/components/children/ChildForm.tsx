  ...
  return (
    <div className="space-y-4">
      ...
      {contacts.map((contact, index) => {
        // Use a unique key based on contact fields (fallback to index if all fields are empty)
        const key = contact.phone || contact.name || contact.relationship || `contact-${index}`;
        return (
...
export default function ChildForm({ child, mode, onSuccess, onCancel }: Readonly<ChildFormProps>) {
  ...
  const submitButtonLabel = mode === 'create' ? 'Crear Niño' : 'Guardar Cambios';
